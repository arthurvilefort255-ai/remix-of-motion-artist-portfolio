          ...
          {inspiracoes.map((pessoa, index) => (
            <motion.article key={pessoa.slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="border-t border-border pt-8">
              <div className="grid gap-10 md:grid-cols-[minmax(240px,0.75fr)_1.25fr] md:gap-16">
                {pessoa.retrato ? <img src={pessoa.retrato} alt={pessoa.retratoAlt} loading="lazy" className="aspect-[4/5] w-full object-cover" /> : <div className="flex aspect-[4/5] items-center justify-center bg-muted" role="img" aria-label={pessoa.retratoAlt}><span className="max-w-[14rem] px-6 text-center text-xs uppercase tracking-[2px] text-muted-foreground">Retrato a adicionar</span></div>}
              ...
              <section className="mt-16"><h3 className="mb-7 text-xl font-medium">Fotos famosas</h3><div className="grid gap-5 md:grid-cols-3">{pessoa.obras.map((obra, obraIndex) => <article key={`${obra.titulo}-${obraIndex}`}>{obra.imagem ? <img src={obra.imagem} alt={obra.titulo} loading="lazy" className="aspect-[4/3] w-full bg-muted object-contain" /> : <div className="flex aspect-[4/3] items-center justify-center bg-muted" role="img" aria-label={`Espaço reservado para ${obra.titulo}`}><span className="px-4 text-center text-xs uppercase tracking-[1px] text-muted-foreground">Imagem protegida por direitos autorais</span></div>}<div className="border-b border-border py-4"><h4 className="font-medium">{obra.titulo}{obra.ano ? ` (${obra.ano})` : ""}</h4><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{obra.legenda}</p>{obra.link ? <a href={obra.link} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-sm underline underline-offset-4">Ver a obra <ExternalLink size={14} aria-hidden="true" /></a> : <span className="mt-4 block text-xs text-muted-foreground">Link a adicionar</span>}</div></article>)}</div></section>
            </motion.article>
          ))}
        </div>
      </section>
