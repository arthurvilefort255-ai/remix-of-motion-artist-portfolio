    ...
    <div className="flex w-full flex-col md:w-1/2 lg:w-1/3">
      {fotos.map((foto, index) => (
        <motion.div key={foto.arquivo} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1 }} transition={{ duration: 0.7, delay: index % 2 === 0 ? atraso : atraso + 0.1 }}>
          <Link to={`/fotos/${foto.sessaoSlug}?foto=${encodeURIComponent(foto.arquivo.split("/").pop() ?? "")}`} className="group relative block aspect-[4/5] overflow-hidden">
            <img src={foto.arquivo} alt={`${foto.titulo}, sessão ${foto.sessaoNome}`} width={1280} height={1600} loading="lazy" className="h-full w-full object-cover transition-opacity duration-300 group-hover:opacity-90" />
            <span className="absolute bottom-0 left-0 bg-background/85 px-3 py-2 text-xs uppercase tracking-[1px] text-foreground">{foto.sessaoNome}</span>
          </Link>
        </motion.div>
      ))}
    </div>
...
export function PhotoGrid({ fotos }: { fotos: FotoComSessao[] }) {
  ...
  return (
    <section className="relative mx-auto flex w-full max-w-[1365px] flex-col items-center" aria-label="Seleção de sessões">
